...
//   ~WintexCommand() { ESP_LOGV("WintexCommand", "WintexCommand Destructor called for %c : %s", cmd, format_hex_pretty(data).c_str()); }
// };
...
struct AsyncWintexCommand {
  ...
  AsyncWintexCommand(WintexCommandType cmd, std::vector<uint8_t> &data,
    ResponseCallback callback) : cmd{cmd}, data{data}, callback{callback} {};
  AsyncWintexCommand(WintexCommandType cmd, ResponseCallback callback) : 
    cmd{cmd}, data{}, callback{callback} {};
  AsyncWintexCommand() : cmd{WintexCommandType::NONE}, data{} {}
  ~AsyncWintexCommand() { ESP_LOGE("AsyncWintexCommand", "AsyncWintexCommand Destructor called for %c : %s", static_cast<uint8_t>(cmd), format_hex_pretty(data).c_str()); }
};
...
class WintexButton : public button::Button {
  ...
  private:
    ...
    AsyncWintexCommand commit_ = AsyncWintexCommand(
      WintexCommandType::COMMIT, this->commit_callback_);
